    ...
    <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-9">
      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <aside className="h-fit rounded-xl border border-border bg-surface p-5 lg:sticky lg:top-24">
          ...
          {selectedSlotIds.length > 0 ? <Link href={`/booking?date=${activeDay.key}&court=${selectedCourtId}&slots=${encodeURIComponent(selectedTimes.join(","))}`} className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-5 py-3.5 font-extrabold text-primary-foreground">Lanjutkan <ArrowRight className="size-5" /></Link> : <button type="button" disabled className="mt-5 w-full rounded-lg bg-surface-high px-5 py-3.5 font-bold text-muted/50">Pilih jadwal dulu</button>}
        </aside>
      </div>
    </div>
  );
