...
export function BookingForm({ court, date, times }: { court: CourtSummary; date: string; times: string[] }) {
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8 lg:py-9">
      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        <form onSubmit={submit} className="rounded-xl border border-border bg-surface p-5 sm:p-7">
          ...
          <p className="mt-3 text-center text-xs leading-5 text-muted">Dengan melanjutkan, jadwal akan ditahan selama 15 menit untuk pembayaran.</p>
        </form>
        <aside className="h-fit overflow-hidden rounded-xl border border-border bg-surface lg:sticky lg:top-24"><div className="relative aspect-[16/8]"><Image src={court.imageUrl} alt={court.name} fill sizes="340px" className="object-cover" /></div><div className="p-5"><p className="text-xs font-bold uppercase tracking-widest text-muted">Rincian lapangan</p><h2 className="mt-2 text-2xl font-bold">{court.name}</h2><p className="mt-1 text-sm text-muted">{court.floorType}</p><div className="mt-5 grid gap-3 border-y border-border py-4 text-sm"><p className="flex items-center gap-3"><CalendarDays className="size-4 text-primary" /><span className="capitalize">{formattedDate}</span></p><p className="flex items-center gap-3"><Clock3 className="size-4 text-primary" />{times.join(", ")} · {times.length} jam</p></div><div className="mt-5 flex items-end justify-between"><span className="text-sm text-muted">Total</span><strong className="text-2xl text-primary">{rupiah.format(court.hourlyRate * times.length)}</strong></div></div></aside>
      </div>
    </div>
  );
}
