...
type CourtCardProps = {
  court: { id: string; name: string; floorType: string; location: string; hourlyRate: number; availableSlots: number; imageUrl: string };
};
  ...
  return (
    <article className="group overflow-hidden rounded-xl border border-border bg-surface transition hover:border-primary">
      <div className="relative aspect-[16/9] overflow-hidden bg-surface-high">
        ...
        <span className="absolute right-3 top-3 inline-flex items-center gap-2 rounded-full bg-primary px-3 py-1.5 text-xs font-extrabold text-primary-foreground"><span className="size-2 rounded-full bg-primary-foreground" />Tersedia</span>
      </div>
      <div className="p-5">
        ...
        <div className="mt-5 flex items-center justify-between gap-4 border-t border-border pt-4">
          ...
          <Link href={`/jadwal?court=${court.id}`} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-extrabold text-primary-foreground transition hover:brightness-110">Lihat Jadwal <ArrowRight className="size-4" /></Link>
        </div>
      </div>
    </article>
  );
